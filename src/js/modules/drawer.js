...
export const drawer = (toggleElement, drawerElementTarget) => {
  ...
  const closeLabelElement = toggleElement.querySelector('[data-close-label]')
  const closeLabel = closeLabelElement?.dataset.closeLabel || 'close' // トグルのラベル（閉じる）
  const openLabelElement = toggleElement.querySelector('[data-open-label]')
  const openLabel = openLabelElement?.dataset.openLabel || 'open' // トグルのラベル（閉じる）
  const methods = {}
  ...
  // hundle
  methods.onToggle = (event, callback) => {
    event.preventDefault()
    methods.toggle(callback)
  }

  // Escapeキーでドロワーを閉じる
  methods.onKeydownEsc = (event, callback) => {
  ...
  methods.addHandleEvent = (selfDrawerElement = drawerElement, callback) => {
    toggleElement?.addEventListener(
      'click',
      (e) => {
        methods.onToggle(e, callback)
      },
      false,
    )
    window.addEventListener(
      'keydown',
      (e) => {
        methods.onKeydownEsc(e, callback)
      },
      false,
    )
  ...
  methods.removeHandleEvent = (selfDrawerElement = drawerElement, callback) => {
...
export const createDrawer = (cloneTarget, mqlConditions = '(min-width: 1023px)') => {
  ...
  const logicAccordionSelector = '#global-menu-drawer .js-drawerInAccordion'
  const accordionElems = []
  const drawerInAccordionMql = window.matchMedia('(min-width:1024px)')
  const handleDrawerInAccordionMql = (mql, accordionElems) => {
    if (mql.matches) {
  ...
  const handleMql = (mql) => {
    ...
    if (mql.matches) {
      ...
      mqlConditions !== null && cloneTarget.remove() // cloneTarget要素削除
      ...
      document.querySelectorAll(logicAccordionSelector)?.forEach((accordionElem) => {
        ...
        accordionElems.push(Accordion)
      })

      handleDrawerInAccordionMql(drawerInAccordionMql, accordionElems)
      drawerInAccordionMql.addEventListener('change', (mql) => {
        handleDrawerInAccordionMql(mql, accordionElems)
      })
    }
  }
